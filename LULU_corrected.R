...
#curated_results<-lulu2(otutab, matchlist)
#You still need to generate the otutab and matchlist objects as defined in the LULU documentation
          ...
          minimum_ratio = 1, minimum_match = 84, minimum_relative_cooccurence = 0.95) 
{
 ...
 #log_con <- file(paste0("lulu.log_", format(start.time,"%Y%m%d_%H%M%S")), open = "a")
  log_con <- file(paste0("lulu_bug_corrected.log_", format(start.time, 
                                             "%Y%m%d_%H%M%S")), open = "a")
        ...
        if (!success) {
          ...
          if (relative_cooccurence >= minimum_relative_cooccurence) {
            ...
            if (minimum_ratio_type == "avg") {
              ...
              #The above commented lines are replaced by:
